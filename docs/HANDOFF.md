...
course "IT Frontiers Book F", 4 units / 17 lessons / 106 steps / 19 published MCQs). Both
externally-generated packages validated perfectly (0 schema errors; 12/12 code steps execute
byte-exact); the winner was chosen for fidelity (164 coverage items, only 3 skips — hands-on book
activities became assignment steps instead of being dropped) and depth (106 steps, original lesson
builds). tools/import_book_manifest.py validates manifests fail-closed (incl. executing every
practice/code step through the real interpreter) and emits the idempotent seed
(20260711000000_itf_book_f_seed.sql, 166 upserts, mode-pinned shapes matching upsert_step,
lesson spine 101-117). Adversarial review applied the seed twice against a real Postgres 16 +
anon RLS probes; folds: seed added to the deploy workflow's hardcoded migration whitelist (it
would have shipped dark), is_current not re-forced on redeploys, updated_at touched on structural
upserts. First deploy failed with curl "Argument list too long" (the 254KB seed exceeded the
kernel's per-argument limit) — fixed by passing migration bodies via temp file
(--data-binary @file); re-run green (migrations step + edge functions all deployed).
Files changed: docs/curriculum/bookf/*.json, tools/import_book_manifest.py,
supabase/migrations/20260711000000_itf_book_f_seed.sql, .github/workflows/deploy-backend.yml,
docs/HANDOFF.md.
Tests run: validator (0 errors, all code steps executed against the engine); reviewer's
double-apply on Postgres 16 + RLS probes as anon/authenticated; deploy run green.
Remaining concerns: Book F rows are manifest-owned — studio edits to itf-* rows are overwritten
on every deploy (regenerate the manifest instead); direct Supabase REST verification from this
container is blocked by the network policy (verified via the deploy workflow's per-file
fail-loudly contract instead); published MCQ answer keys are anon-readable (pre-existing platform
design, now ×10 surface).
Suggested next task: user live pass (student catalog + teacher class Structure section scoped to
the course); then repeat the pipeline for the next IT Frontiers book.
