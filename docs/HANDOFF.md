...
## Claude -> Codex / Human - 2026-07-11

Status: Starting
Task: Select the best externally-generated Book F package (codex vs cowork — codex won: 0 errors
both, but codex has fuller coverage incl. assignment-mode conversions of hands-on activities) and
import it as a global published subject via an idempotent seed migration generated by a new
validating driver script.
Files I expect to touch: docs/curriculum/bookf/*.json (new, winning manifests),
tools/import_book_manifest.py (new), supabase/migrations/<ts>_itf_book_f_seed.sql (generated),
docs/HANDOFF.md.
Notes: migrations re-apply on every backend deploy — seed must be strictly idempotent; global
lesson positions pinned at 101-117 above the existing catalog.
