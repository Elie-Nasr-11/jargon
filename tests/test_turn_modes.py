...
class DeclaredTurnMode(unittest.TestCase):
    def test_mode_vocabulary_is_closed_and_excludes_checkpoints(self):
        ...
        for mode in ("lesson", "practice", "discuss", "quiz", "assignment", "open"):
    ...
    def test_unknown_mode_degrades_to_legacy(self):
        fn = CHAT[CHAT.index("function studentTurnMode(") :][:400]
    ...
    def test_ceiling_lifts_null_and_attempt_kinds_for_conversation_modes(self):
        ...
        fn = fn[: fn.index("\n}")]
        # Only discuss/open are capped; every other mode passes the router verdict through.
        self.assertIn('mode !== "discuss" && mode !== "open"', fn)
    ...
    def test_control_turns_bypass_the_ceiling(self):
        ...
        site = CHAT[CHAT.index("const routedKind: RoutedKind | null = controlType") :][:400]
