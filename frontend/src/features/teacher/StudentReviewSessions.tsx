...
export function StudentReviewSessions({ studentId }: { studentId: string }) {
  ...
  return (
    <div className="rounded-card border border-border bg-depth-sub p-4">
      <div className="text-overline font-medium uppercase tracking-[0.08em] text-muted-foreground">
        Spaced review
      </div>
      <ul className="mt-2 space-y-1.5">
        {rows.slice(0, 10).map((row) => (
          <li key={row.id} className="flex items-center gap-2.5 text-meta">
            ...
            <span
              className={`shrink-0 rounded-full border px-2 py-0.5 text-overline ${
                row.status === "complete"
            ...
            <span className="w-10 shrink-0 text-right text-overline text-muted-foreground">
