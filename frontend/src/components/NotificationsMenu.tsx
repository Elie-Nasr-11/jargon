  ...
  const openItem = (n: Notification) => {
    ...
    if (n.kind === "direct_message" && n.class_id && n.related_student_id) {
      ...
      navigate({
        ...
        search: assignmentId
          ? { tab: "today", assignment: assignmentId }
          : assessmentId
            ? { tab: "today", assessment: assessmentId }
            : { tab: "today" },
      });
  ...
  return (
    <div ref={wrapRef} className="relative">
      <button
        ...
        className="relative flex h-11 w-11 items-center justify-center rounded-full bg-depth-card text-muted-foreground shadow-card transition-colors hover:text-foreground sm:h-9 sm:w-9"
      >
